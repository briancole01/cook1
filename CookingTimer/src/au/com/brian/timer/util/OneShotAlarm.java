...
public class OneShotAlarm extends BroadcastReceiver
{
    ...
    public void onReceive(Context context, Intent intent)
    {
    	...
    	//final Message m = mHandler.obtainMessage(MSG_ID);
    	//m.arg1 = next.getId();
    	//Bundle data = new Bundle();
    	//data.putLong("finishTime", next.getAlarmTime().getTimeInMillis());
		//m.setData(data);
		...
		//			e.printStackTrace();
		//		}
        //    	Log.i(TAG, "sending args = " +  m.arg1 + "," +  m.arg2);
    ...
    private PendingIntent getPendingIntent(int id) {
        Intent intent = new Intent(context, OneShotAlarm2.class);
    ...
    private Handler mHandler = new Handler() {
        @Override public void handleMessage(Message m) {
        	Log.i(TAG, "handler args = " +  m.arg1 + "," +  m.arg2 + "," +  m.getData().getLong("finishTime"));
        	int id = m.arg1;
        	long finishTime = m.getData().getLong("finishTime");
       	    setAlarm(id,finishTime);
        }
